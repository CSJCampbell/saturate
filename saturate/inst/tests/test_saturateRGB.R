...
test_that("hit remaining symbols", {
          ...
          expect_that(saturateHSV(eg1, fixed = 0.5),
                      throws_error("three rows corresponding to hue, saturation and value expected in mat"))

          eg2 <- matrix(letters[1:6], nrow = 3, ncol = 2)

          expect_that(saturateHSV(eg2, sat = 2),
                      throws_error("mat must be numeric"))

          eg3 <- matrix((1:6) * 32, nrow = 3, ncol = 2)

          expect_that(saturateHSV(eg3, sat = NA),
                      throws_error("sat must be numeric"))

          expect_that(saturateHSV(eg3, fixed = NA),
                      throws_error("fixed must be numeric"))
          ...
          eg7 <- matrix(c(123, -101, 55, 123, 101, 55, 123, 356, 55),
                        nrow = 3, ncol = 3)
